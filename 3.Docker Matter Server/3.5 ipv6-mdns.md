...
mDNS Discovery: Home Assistant discovers Thread Border Routers through mDNS announcements. Ensure your Docker network configuration allows for mDNS traffic to be broadcast and received correctly. 

This often requires using a macvlan network for the OTBR Docker container to give it direct network access, or ensuring proper multicast routing if using a bridged network.

Firewall Rules: Verify that no firewall rules on your host system or network are blocking mDNS (UDP port 5353) or other necessary Thread communication
